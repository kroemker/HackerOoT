 ...
 * Overlay: ovl_Transform_Ik
 * Description: Play as an Iron Knuckle (ported from the hm-comp-2 branch)
 */
...
ActorProfile Transform_Ik_Profile = {
    /**/ ACTOR_TRANSFORM_IK,
    /**/ ACTORCAT_ITEMACTION,
    /**/ FLAGS,
    /**/ OBJECT_IK,
...
void TransformIk_SetupAction(TransformIk* this, PlayState* play, TransformIkActionFunc actionFunc) {
    ...
    if (this->actionFunc == TransformIk_Action_Cutscene) {
        ...
        Animation_Change(&this->skelAnime, this->recoverAnimation, 1.5f, 0.0f,
                         Animation_GetLastFrame(this->recoverAnimation), ANIMMODE_ONCE_INTERP, -4.0f);
        Interface_SetDoAction(play, DO_ACTION_NONE);
        Interface_LoadActionLabelB(play, DO_ACTION_NONE);
...
static Vec3f sShieldTris0[] = {
    { -3000.0, -700.0, -5000.0 },
    { -3000.0, -700.0, 2000.0 },
    { 4000.0, -700.0, 2000.0 },
};

static Vec3f sShieldTris1[] = {
    { 4000.0, -700.0, 2000.0 },
    { 4000.0, -700.0, -5000.0 },
    { -3000.0, -700.0, -5000.0 },
};
...
void TransformIk_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == IRON_KNUCKLE_LIMB_HELMET_ARMOR) {
        ...
        Vec3f shieldVertices1[3];

        Matrix_MultVec3f(&sAxeQuadOffsets[0], &this->axeCollider.dim.quad[1]);
        Matrix_MultVec3f(&sAxeQuadOffsets[1], &this->axeCollider.dim.quad[0]);
        Matrix_MultVec3f(&sAxeQuadOffsets[2], &this->axeCollider.dim.quad[3]);
        Matrix_MultVec3f(&sAxeQuadOffsets[3], &this->axeCollider.dim.quad[2]);
        ...
        Matrix_MultVec3f(&sAxeQuadOffsets[0], &blureP1);
        Matrix_MultVec3f(&sAxeQuadOffsets[1], &blureP2);
        if (this->attackState > 0) {
            EffectBlure_AddVertex(Effect_GetByIndex(this->blureIdx), &blureP1, &blureP2);
        } else if (this->attackState == 0) {
            EffectBlure_AddSpace(Effect_GetByIndex(this->blureIdx));
        }
        if (this->shieldState > 0) {
            for (i = 0; i < ARRAY_COUNT(shieldVertices1); i++) {
                Matrix_MultVec3f(&sShieldTris0[i], &shieldVertices0[i]);
                Matrix_MultVec3f(&sShieldTris1[i], &shieldVertices1[i]);
            }

            Collider_SetTrisVertices(&this->shieldCollider, 0, &shieldVertices0[0], &shieldVertices0[1],
                                     &shieldVertices0[2]);
            Collider_SetTrisVertices(&this->shieldCollider, 1, &shieldVertices1[0], &shieldVertices1[1],
                                     &shieldVertices1[2]);
        }
    }
